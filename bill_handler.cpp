...
QJsonObject bill_handler::handleSyncBills(const QJsonObject& request)
{
    ...
    for (const AccountRecord& record : recordsToSync) {
        int localId = record.getId();  // 客户端本地ID（新增时为0）
        int serverId = m_accountManager->addAccountRecord(record);
        if (serverId > 0) {
            successCount++;
            // 添加ID映射到响应数组
            QJsonObject idMapping;
            idMapping["localId"] = localId;
            idMapping["serverId"] = serverId;
            billsResponseArray.append(idMapping);
        } else {
            failCount++;
        }
    }
    ...
    if (successCount > 0) {
        ...
        response["message"] = QString("同步成功：%1条记录").arg(successCount);
        response["successCount"] = successCount;
        response["failCount"] = failCount;
        // 如果有新插入的记录，返回ID映射
        if (!billsResponseArray.isEmpty()) {
            response["bills"] = billsResponseArray;
        }
    } else {
        db.rollback();
        response["success"] = false;
